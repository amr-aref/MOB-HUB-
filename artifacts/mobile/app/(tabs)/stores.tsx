  ...
  return (
    <View style={styles.container}>
      ...
      <View style={[styles.statsBar, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
        <View style={isTablet ? [styles.tabletCentered, { flexDirection: isRTL ? 'row-reverse' : 'row', justifyContent: 'space-between', flex: 1 }] : { flex: 1, flexDirection: isRTL ? 'row-reverse' : 'row', justifyContent: 'space-between' }}>
      ...
      >
        {filtered.length === 0 ? (
          ...
          <View style={[isTablet && styles.tabletGrid, isTablet && { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
            {filtered.map((store) => (
              <View key={store.id} style={[styles.cardWrap, isTablet && styles.tabletGridItem]}>
                <StoreCard
                  store={store}
                  listMode
