...
router.post("/auth/register", async (req, res) => {
  ...
  // Public registration always creates a buyer. Merchant/admin roles and store
  // ownership are assigned only through authenticated administrative workflows.
  const { user, tokens } = await register(v.data, req.ip, req.headers["user-agent"]);

  await logAuthEvent({ userId: user.id, event: "register", success: true, req });
...
router.post("/auth/login", async (req, res) => {
  ...
  try {
    const { user, tokens } = await login(v.data, req.ip, req.headers["user-agent"]);
    userId = user.id;
    ...
    await logAuthEvent({
      ...
      event: "login",
      success: false,
      metadata: { email: v.data.email, reason: err instanceof AppError ? err.code : "unknown" },
      req,
    });
    throw err;
  }
...
router.post("/auth/refresh", async (req, res) => {
  ...
  await logAuthEvent({ event: "token_refresh", success: true, req });
  res.json(tokens);
});

router.post("/auth/logout", requireAuth, async (req, res) => {
  await logout(req.user!.sessionId);
  await logAuthEvent({ userId: req.user!.sub, event: "logout", success: true, req });
  res.status(204).send();
});

router.post("/auth/logout-all", requireAuth, async (req, res) => {
  await logoutAll(req.user!.sub);
  await logAuthEvent({ userId: req.user!.sub, event: "logout", success: true, metadata: { allDevices: true }, req });
  res.status(204).send();
});
...
router.post("/auth/forgot-password", async (req, res) => {
  ...
  await logAuthEvent({
    ...
    success: true,
    metadata: { tokenIssued: resetToken !== null },
    req,
  });
  res.json({
    message: "If an account with that email exists, a reset link has been sent.",
    ...(process.env.NODE_ENV !== "production" && resetToken ? { _devToken: resetToken } : {}),
  });
});
...
router.post("/auth/reset-password", async (req, res) => {
  ...
  await logAuthEvent({ event: "password_reset_completed", success: true, req });
...
router.post("/auth/send-verification", requireAuth, async (req, res) => {
  ...
  await logAuthEvent({ userId: req.user!.sub, event: "email_verification_sent", success: true, req });
  res.json({
    message: "Verification email sent.",
    ...(process.env.NODE_ENV !== "production" ? { _devToken: verifyToken } : {}),
  });
});
...
router.post("/auth/verify-email", async (req, res) => {
  ...
  await logAuthEvent({ event: "email_verified", success: true, req });
