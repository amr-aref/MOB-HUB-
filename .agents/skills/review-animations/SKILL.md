...
- Duration > 300ms on a standard UI element (not marketing/onboarding)
- Any animation on a keyboard-initiated or 100+/day action
- `width`/`height`/`top`/`left`/`margin` in a transition or keyframe
- Framer Motion `x`/`y`/`scale` shorthand props used at scale
- No `prefers-reduced-motion` handling on motion that moves things
